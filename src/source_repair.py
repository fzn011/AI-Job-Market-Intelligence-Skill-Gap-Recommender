...
UI_THEME_EXPORTS: tuple[str, ...] = (
    "apply_global_theme", "render_brand_header", "render_feature_card", "render_info_box",
    "render_path_card", "render_sidebar_navigation", "render_status_badge", "render_app_footer", "style_plotly_figure",
)
...
GIT_REPAIR_PATHS = ["src/ui_theme.py", "src/_repair/ui_theme.py", "src/brand_constants.py", "src/source_repair.py", "src/__init__.py", "app/streamlit_app.py"]
MIN_UI_THEME_BYTES = 12_000
...
def ui_theme_imports_work(root: Path) -> bool:
    ...
    try:
        ...
        return False
    return all(hasattr(module, n) for n in UI_THEME_EXPORTS)

def ui_theme_is_valid(root: Path) -> bool:
    return file_has_markers(root, "src/ui_theme.py") and ui_theme_file_large_enough(root) and ui_theme_imports_work(root)
...
def ensure_careercompass_sources(root: Path | None = None) -> None:
    ...
    try:
        ...
        if not ui_theme_is_valid(root):
            ...
            clear_python_cache(root)
        if not ui_theme_is_valid(root):
            raise RuntimeError("CareerCompass source repair failed for src/ui_theme.py (missing render_feature_card). Run: python scripts/emergency_repair.py")
    finally:
        _repair_running = False
...
def repair_report(root: Path | None = None) -> list[str]:
    ...
    lines.append(f"ui_theme.py size: {ui_path.stat().st_size if ui_path.exists() else 'MISSING'} bytes (min {MIN_UI_THEME_BYTES})")
